    ...
    return new Promise((resolve, reject) => {
        getMaxPage(page, query.keyword, "tb_products").then(maxPage => {
            ...
            conn.query(`${sql} LIMIT ? OFFSET ?`,
                query.keyword == null ? [page.limit, page.offset] : ['%' + query.keyword + '%', page.limit, page.offset],
                (err, data) => {
                    if (!err) resolve({
                        infoPage,
                        data
...
exports.newProduct = req => {
    const body = req.body;
    return new Promise((resolve, reject) => {
        conn.query(`INSERT INTO tb_products SET name = ?, description = ?, image = ?, category = ?, price = ?, quantity = ?`,
            [body.prod_name, body.prod_desc, body.prod_image, body.category_id, body.price, body.quantity],
            (err, result) => {
                if (!err) resolve(result);
                else reject(err);
            }
...
exports.updateProduct = req => {
    const body = req.body;
    return new Promise((resolve, reject) => {
        conn.query(`UPDATE tb_products SET name = ?, description = ?, image = ?, category = ?, price = ?, quantity = ? WHERE id = ?`,
            [body.prod_name, body.prod_desc, body.prod_image, body.category_id, body.price, body.quantity, req.params.prod_id],
            (err, result) => {
                if (!err) resolve(result);
                else reject(err);
            }
...
exports.deleteProduct = req => {
    return new Promise((resolve, reject) => {
        ...
        conn.query(`DELETE FROM tb_products WHERE id = ?`, [prodId], (err, result) => {
            if (!err) resolve(result);
            else reject(err);
        });
    });
